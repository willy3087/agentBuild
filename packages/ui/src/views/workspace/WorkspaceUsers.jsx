...
const WorkspaceDetails = () => {
    ...
    const addUser = () => {
        const dialogProp = {
            type: 'ADD',
            cancelButtonName: t('workspace.users.dialog.invite.buttons.cancel'),
            confirmButtonName: t('workspace.users.dialog.invite.buttons.confirm'),
            data: workspace
        }
    ...
    const editInvite = (user) => {
        const dialogProp = {
            type: 'EDIT',
            cancelButtonName: t('workspace.users.dialog.updateInvite.buttons.cancel'),
            confirmButtonName: t('workspace.users.dialog.updateInvite.buttons.confirm'),
            data: {
                ...user,
    ...
    const editUser = (user) => {
        ...
        const dialogProp = {
            type: 'EDIT',
            cancelButtonName: t('workspace.users.dialog.updateRole.buttons.cancel'),
            confirmButtonName: t('workspace.users.dialog.updateRole.buttons.confirm'),
            data: userObj
        }
    ...
    const unlinkUser = async () => {
        ...
        const confirmPayload = {
            title: t('workspace.users.dialog.remove.title'),
            description: t('workspace.users.dialog.remove.description', { userList }),
            confirmButtonName: t('workspace.users.dialog.remove.buttons.confirm'),
            cancelButtonName: t('workspace.users.dialog.remove.buttons.cancel')
        }
        ...
        if (orgOwner) {
            enqueueSnackbar({
                message: t('workspace.users.notifications.cannotRemoveOwner'),
                options: {
        ...
        if (isConfirmed) {
            try {
                ...
                enqueueSnackbar({
                    message: t('workspace.users.notifications.removed', { count: usersSelected.length }),
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
                ...
                enqueueSnackbar({
                    message: t('workspace.users.notifications.unlinkFailed', {
                ...
                {error ? (
                    ...
                    <Stack flexDirection='column' sx={{ gap: 3 }}>
                        <ViewHeader
                            ...
                            searchPlaceholder={t('workspace.users.search.placeholder')}
                            title={t('workspace.users.title', { workspaceName: workspace?.name || '' })}
                            description={t('workspace.users.description')}
                        >
                            {workspaceUsers.length > 0 && (
                                <>
                                    ...
                                    >
                                        {t('workspace.users.actions.removeUsers')}
                                    </PermissionButton>
                                    ...
                                    >
                                        {t('workspace.users.actions.addUser')}
                                    </StyledPermissionButton>
                                </>
                            )}
                        </ViewHeader>
                        {!isLoading && workspaceUsers?.length <= 0 ? (
                            <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                                ...
                                <div>{t('workspace.users.empty.noUsers')}</div>
                                <StyledPermissionButton
                                    permissionId={'workspace:add-user'}
                                    variant='contained'
                                    ...
                                    onClick={addUser}
                                >
                                    {t('workspace.users.empty.addUser')}
                                </StyledPermissionButton>
                            </Stack>
                        ) : (
                            <>
                                            ...
                                            <TableRow>
                                                <StyledTableCell padding='checkbox'>
                                                    <Checkbox
                                                        ...
                                                        inputProps={{
                                                            'aria-label': t('workspace.users.table.selectAll')
                                                        }}
                                                    />
                                                </StyledTableCell>
                                                <StyledTableCell>{t('workspace.users.table.emailName')}</StyledTableCell>
                                                <StyledTableCell>{t('workspace.users.table.role')}</StyledTableCell>
                                                <StyledTableCell>{t('workspace.users.table.status')}</StyledTableCell>
                                                <StyledTableCell>{t('workspace.users.table.lastLogin')}</StyledTableCell>
                                                ...
                                                <>
                                                    {(workspaceUsers || []).filter(filterUsers).map((item, index) => (
                                                        ...
                                                        >
                                                            ...
                                                            <StyledTableCell>
                                                                {item.isOrgOwner ? (
                                                                    <Chip size='small' label={t('workspace.users.table.roles.orgOwner')} />
                                                                ) : (
                                                                    item.role.name
                                                                )}
                                                            </StyledTableCell>
                                                            ...
                                                            <StyledTableCell>
                                                                ...
                                                                {!item.isOrgOwner && item.status.toUpperCase() === 'ACTIVE' && (
                                                                    <IconButton
                                                                        title={t('workspace.users.table.actions.changeRole')}
                                                                        color='primary'
                                                                        onClick={() => onEditClick(item)}
                                                                    >
                                                                        <IconEdit />
